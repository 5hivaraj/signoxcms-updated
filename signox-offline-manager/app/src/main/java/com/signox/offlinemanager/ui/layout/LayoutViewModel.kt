...
class LayoutViewModel(
    ...
    fun createLayout(name: String, description: String, width: Int = 1920, height: Int = 1080, template: LayoutTemplate? = null) {
        viewModelScope.launch {
            try {
                ...
                template?.let { tmpl ->
                    createZonesFromTemplate(layoutId, tmpl)
                }
            } catch (e: Exception) {
    ...
    private suspend fun createZonesFromTemplate(layoutId: Long, template: LayoutTemplate) {
        template.sections.forEach { section ->
            val zone = com.signox.offlinemanager.data.model.LayoutZone(
                layoutId = layoutId,
                name = section.name,
                x = (section.x * template.width / 100).toInt(),
                y = (section.y * template.height / 100).toInt(),
                width = (section.width * template.width / 100).toInt(),
                height = (section.height * template.height / 100).toInt(),
