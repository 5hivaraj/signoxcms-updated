...
class LayoutEditorViewModel(
    ...
    fun loadLayout(layoutId: Long) {
        ...
        viewModelScope.launch {
            try {
                ...
                layoutRepository.getLayoutZones(layoutId).observeForever { zones ->
                    _zones.value = zones
                }
    ...
    fun createZone(x: Int, y: Int, width: Int, height: Int) {
        val currentLayout = _layout.value ?: return
        val zoneCount = _zones.value?.size ?: 0
        ...
        viewModelScope.launch {
            try {
                ...
                _error.value = e.message ?: "Failed to create zone"
            }
        }
    }
